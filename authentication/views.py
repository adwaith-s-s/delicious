...
class ProfileView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        order = Order.objects.filter(user=request.user.userprofile)
        delivered = Order.objects.filter(user=request.user.userprofile, status='Order Delivered')
        context = {'order': order, 'delivered':delivered}
        return render(request, self.template_name, context)
...
class CartPage(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for x in cart:
            total += x.total
        context = {'cart': cart, 'total':total, 'shipping_address':shipping_address}
        return render(request, self.template_name, context)
